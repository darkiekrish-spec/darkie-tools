...
mkdir -p "$DIR/dist"
cd "$DIR/build"
hdiutil create -volname "DarkieSuite" -srcfolder DarkieSuite.app -ov -format UDZO "$DIR/dist/DarkieSuite-v5-macos.dmg" 2>/dev/null || echo "hdiutil not available (macOS only). App bundle created at $APP"
